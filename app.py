...
if uploaded_file is not None:
    ...
    fig_data, ax_data = plt.subplots(figsize=(12, 5))
    ...
    if submit_button:
        ...
        def objective_function(particles):
            ...
            for i, particle in enumerate(particles):
                ...
                units = int(np.round(particle[0]))
                lr = float(particle[1])
                batch = int(np.round(particle[2]))
                ...
                try:
                    ...
                    y_val_true_inv = scaler_y.inverse_transform(
                        y_val.reshape(-1, 1)
                    ).flatten()
        ...
        ax_conv.plot(
            range(1, len(optimizer.cost_history)+1),
